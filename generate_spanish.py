...
# Skip sitemap - main sitemap.xml already includes all URLs
SITEMAP_FILE = None
BASE_URL = "https://plantfinder.org/es"

def get_light_label(level):
    labels = {1: "Muy Bajo", 2: "Bajo", 3: "Medio (Indirecto)", 4: "Brillante Indirecto", 5: "Sol Directo"}
...
def generate_plant_html(plant):
    plant_id = plant["id"]
    name = plant.get("name_es", plant["name"])  # Use Spanish name if available
    ...
    if not pet_safe and toxic_to:
        ...
        pet_warning_html = f'''
        <!-- Segura para Mascotasty Warning -->
        <div class="bg-rose-50 border border-rose-200 rounded-2xl p-6 mb-8">
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-rose-700">{name} contiene compuestos tóxicos to {pets_str} if ingested. Keep this plant out of reach of pets, or consider a <a href="/search/?pet_safe=true" class="underline hover:no-underline">alternativa segura para mascotas</a>.</p>
                </div>
            </div>
        </div>
'''
...
'''
    ...
    if air_purifying:
        info_badges.append('''<div class="flex items-center gap-2 px-4 py-2 bg-emerald-100 rounded-xl">
                            ...
                            <span class="text-sm font-medium text-emerald-700">Purificación de Aire</span>
                        </div>''')
    ...
    if difficulty == "easy":
        best_for.append("Principiantes y cuidadores ocupados")
    ...
    if size == "Large":
        best_for.append("Hacer una declaración en espacios amplios")
        not_ideal.append("Apartamentos pequeños o espacios reducidos")
    elif size == "Small":
        best_for.append("Escritorios, estantes y espacios pequeños")
    
    if humidity >= 4:
        best_for.append("Baños o habitaciones con humidificadores")
        not_ideal.append("Climas muy secos sin control de humedad")
    
    if air_purifying:
        best_for.append("Mejorar la calidad del aire interior")
    
    if category == "trailing":
        best_for.append("Cestas colgantes y estantes altos")
    elif category == "succulent" or category == "cactus":
        best_for.append("Alféizares soleados")
...
<head>
    ...
    <meta name="description" content="Guía completa de cuidado para {name}{' (' + common_names[0] + ')' if common_names else ''}. Aprende sobre luz, agua, humedad, y cómo mantener tu {name} saludable.">
    ...
    <meta name="twitter:title" content="{name} Guía de Cuidado">
    ...
    {{
        ...
        "headline": "{name}: Complete Guía de Cuidado, Light & Necesidades de Agua",
    ...
    {{
        ...
        "itemListElement": [
            ...
            {{"@type": "ListItem", "position": 2, "name": "Plantas", "item": "{BASE_URL}/search/"}},
...
{pet_warning_html}
        ...
        <section class="bg-gradient-to-br from-emerald-50 via-teal-50 to-lime-50 rounded-2xl p-6 md:p-8 mb-8">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                {f'<a href="/search/?pet_safe=true" class="inline-flex items-center gap-2 bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-emerald-700 transition"><i data-lucide="paw-print" class="w-4 h-4"></i>Find alternativa segura para mascotass</a>' if not pet_safe else ''}
            </div>
        </section>
    </main>
